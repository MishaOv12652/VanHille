...
router.get('/get/unique/courseNums', (req, res, next) => {
    User.getAllUniqueCourseNum((err,uniqueArray) => {
        if (err) {
            res.json({success:false, msg:err});
        } else {
            res.json({success: true, result:uniqueArray });
        }
    });
})

router.get('/get/unique/corresponding/groupNums/:courseNum', (req, res, next) => {
    User.getCorepondingGroupNums(req.params.courseNum, (err,uniqueArray) => {
        if (err) {
            res.json({success:false, msg:err});
        } else {
            res.json({success: true, result:uniqueArray });
